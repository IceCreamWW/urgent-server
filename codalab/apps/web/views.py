...
class CompetitionS3Upload(LoginRequiredMixin, CompetitionCreationMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        # FIXME: only superuser can create competition
        ...
        try:
            ...
            for group in context['groups']:
                for _, scoredata in group['scores']:
                    ...
                    scoredata['count'] = sub.phase.submissions.filter(participant=sub.participant).filter(status__name="Finished").count()
                    if sub.team:
                        scoredata['team_name'] = sub.team.name
                    scoredata['description'] = sub.description
    ...
    def get_context_data(self, **kwargs):
        ...
        for number, participant in enumerate(comp_participants):
            ...
            participant_entry = {
                ...
                'number': number + 1,
                'entries': participant.submissions.filter(status__name="Finished").count(),
                'team_name': team_name,
                'team': team
            }
